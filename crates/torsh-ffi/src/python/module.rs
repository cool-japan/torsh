...
impl PyLinear {
    #[new]
    fn new(in_features: usize, out_features: usize, bias: Option<bool>) -> Self {
        ...
        let weight = Python::with_gil(|py| {
            let data = pyo3::types::PyList::new_bound(py, &weight_data);
            PyTensor::new(
                data.as_ref(),
        ...
        let bias_tensor = if use_bias {
            ...
            let bias_tensor = Python::with_gil(|py| {
                let data = pyo3::types::PyList::new_bound(py, &bias_data);
                PyTensor::new(
                    data.as_ref(),
                    Some(vec![out_features]),
                    Some("f32"),
                    true,
                ).unwrap()
            });
    ...
    fn forward(&self, input: &PyTensor) -> PyResult<PyTensor> {
        ...
        if let Some(ref bias) = self.bias_tensor {
            ...
            for batch_idx in 0..input.shape()[0] {
                for feature_idx in 0..self.out_features {
                    let output_idx = batch_idx * self.out_features + feature_idx;
...
impl PyReLU {
    ...
    fn forward(&self, input: &PyTensor) -> PyResult<PyTensor> {
        ...
        Ok(PyTensor::new(
            Python::with_gil(|py| {
                pyo3::types::PyList::new_bound(py, &result_data).as_ref()
            }),
            Some(input.shape()),
            Some("f32"),
            input.requires_grad(),
...
mod tests {
    ...
    fn test_linear_creation() {
        let linear = PyLinear::new(10, 5, None);

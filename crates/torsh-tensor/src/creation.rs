...
use rand::{Rng, distributions::{Distribution, Standard, Uniform}};
...
pub fn tensor_2d_arrays<T: TensorElement, const M: usize, const N: usize>(data: &[[T; N]; M]) -> Tensor<T> {
    let rows = M;
    let cols = N;
...
pub fn arange<T: TensorElement + std::cmp::PartialOrd + std::ops::Add<Output = T> + Copy>(start: T, end: T, step: T) -> Result<Tensor<T>>
{

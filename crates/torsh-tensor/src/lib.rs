...
enum Operation<T: TensorElement> {
    Leaf,
    Power { input: Arc<Tensor<T>>, exponent: f32 },
    Add { lhs: Arc<Tensor<T>>, rhs: Arc<Tensor<T>> },
    Mul { lhs: Arc<Tensor<T>>, rhs: Arc<Tensor<T>> },
}
...
impl<T: TensorElement> Tensor<T> {
    ...
    where 
        T: FloatElement + Copy + Default + std::ops::Add<Output = T> + std::ops::Sub<Output = T> + std::ops::Mul<Output = T> + std::ops::Div<Output = T>
    {
        ...
        match &self.operation {
            ...
            Operation::Power { input, exponent } => {
                if input.requires_grad {
                    ...
                    let grad_data: Vec<T> = input_data.iter()
                        .map(|&x| {
                    ...
                    let input_grad = Self::from_data(grad_data, input.shape().dims().to_vec(), input.device);
    ...
    pub fn view(&self, shape: &[i32]) -> Result<Self> {
        let new_shape: Result<Vec<usize>> = shape.iter()
            .map(|&d| {
                if d == -1 {
                    // Infer dimension
                    let known_product: usize = shape.iter()
                        .filter(|&&x| x != -1)
                        .map(|&x| x as usize)
                        .product();
                    ...
                    Err(TorshError::InvalidShape(
                        format!("Invalid dimension size: {}", d)
                    ))
                } else {
                    Ok(d as usize)
                }
        ...
        if new_numel != self.numel() {
            return Err(TorshError::InvalidShape(
                format!("Shape {:?} is invalid for tensor of size {}", new_shape, self.numel())
            ));
        }
...
impl<T: TensorElement> std::fmt::Debug for Tensor<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Tensor(shape={:?}, dtype={}, device={})", 
               self.shape().dims(), self.dtype(), self.device)
    }
}
...
pub mod prelude {
    pub use crate::{Tensor, tensor, tensor_2d};

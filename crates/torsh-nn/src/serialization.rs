...
impl ModelState {
    ...
    pub fn add_parameter(&mut self, name: String, tensor: Tensor<f32>) {
        self.parameters.insert(name, SerializableTensor::from(tensor));
    }
    ...
    pub fn save_to_safetensors<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        let safetensors_data = safetensors::serialize(&tensors, &Some(metadata_map))
            .map_err(|e| TorshError::SerializationError(format!("SafeTensors serialization error: {}", e)))?;
    ...
    pub fn load_from_safetensors<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let safetensors = SafeTensors::deserialize(&data)
            .map_err(|e| TorshError::SerializationError(format!("SafeTensors deserialization error: {}", e)))?;
        ...
        for (name, tensor_view) in safetensors.tensors() {
            if tensor_view.dtype() != safetensors::tensor::Dtype::F32 {
                return Err(TorshError::SerializationError(
                    format!("Unsupported dtype: {:?}. Only F32 is currently supported.", tensor_view.dtype())
                ));
            }
...
mod tests {
    ...
    fn test_file_serialization() {
        ...
        assert_eq!(state.metadata.architecture, loaded_state.metadata.architecture);

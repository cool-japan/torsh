...
pub fn leaky_relu(input: &Tensor, negative_slope: f64) -> Tensor {
    ...
    let positive_part = input.maximum(&zero_tensor).unwrap();
    let negative_part = input
        .minimum(&zero_tensor)
        .unwrap()
        .mul_scalar(negative_slope as f32)
        .unwrap();
    positive_part.add(&negative_part).unwrap()
}
...
pub fn elu(input: &Tensor, alpha: f64) -> Tensor {
    let zero_tensor = torsh_tensor::creation::zeros_like(input).unwrap();
    let positive_part = input.maximum(&zero_tensor).unwrap();
    let negative_part = input.minimum(&zero_tensor).unwrap();
    let exp_part = negative_part
        ...
        .unwrap();
    positive_part.add(&exp_part).unwrap()
}

/// Swish activation function (also known as SiLU)
/// Swish(x) = x * sigmoid(x)
pub fn swish(input: &Tensor) -> Tensor {
    let one_tensor = torsh_tensor::creation::ones_like(input).unwrap();
    let neg_input = input.neg().unwrap();
    let exp_neg = neg_input.exp().unwrap();
    let denominator = one_tensor.add(&exp_neg).unwrap();
    let sigmoid = one_tensor.div(&denominator).unwrap();
...
pub fn gelu(input: &Tensor) -> Tensor {
    ...
    let x_cubed = input.pow_scalar(3.0).unwrap();
    let coeff_term = x_cubed.mul_scalar(coefficient).unwrap();
    let sum_term = input.add(&coeff_term).unwrap();
    let inner = sum_term.mul_scalar(sqrt_2_over_pi).unwrap();
    let tanh_part = inner.tanh().unwrap();

    let one_tensor = torsh_tensor::creation::ones_like(input).unwrap();
    let one_plus_tanh = one_tensor.add(&tanh_part).unwrap();
    let half_tensor = torsh_tensor::creation::ones_like(input)
        .unwrap()
        .mul_scalar(0.5)
        .unwrap();

    half_tensor.mul(input).unwrap().mul(&one_plus_tanh).unwrap()
}
...
pub fn mish(input: &Tensor) -> Tensor {
    let one_tensor = torsh_tensor::creation::ones_like(input).unwrap();
    let exp_input = input.exp().unwrap();
    let one_plus_exp = one_tensor.add(&exp_input).unwrap();
    let softplus = one_plus_exp.ln().unwrap();
    let tanh_softplus = softplus.tanh().unwrap();
    input.mul(&tanh_softplus).unwrap()
}
...
pub mod normalization {
    ...
    pub fn layer_norm(input: &Tensor, eps: f64) -> Tensor {
        ...
        let mean = input.mean(Some(&[last_dim]), false).unwrap();
        ...
        let mean_expanded = mean.unsqueeze(-1).unwrap();
        let mean_broadcasted = mean_expanded.expand(&input_shape).unwrap();
        let diff = input.sub(&mean_broadcasted).unwrap();
        ...
        let variance_expanded = variance.unsqueeze(-1).unwrap();
        let variance_broadcasted = variance_expanded.expand(&input_shape).unwrap();
        let eps_tensor = torsh_tensor::creation::ones_like(&variance_broadcasted)
            .unwrap()
            .mul_scalar(eps as f32)
            .unwrap();
        let variance_plus_eps = variance_broadcasted.add(&eps_tensor).unwrap();
        let std = variance_plus_eps.sqrt().unwrap();
        diff.div(&std).unwrap()
    }
    ...
    pub fn graph_norm(input: &Tensor, edge_index: &Tensor, num_nodes: usize) -> Tensor {
        ...
        let edge_index_i32 = edge_index.to_i32_simd().unwrap();
        let edge_index_i64 = edge_index_i32.to_i64_simd().unwrap();
        let edge_data = crate::utils::tensor_to_vec2::<i64>(&edge_index_i64).unwrap();
        ...
        let degree_expanded = degree_tensor
            ...
            .expand(input_shape)
            .unwrap();
        input.mul(&degree_expanded).unwrap()
    }
    ...
    pub fn batch_norm(input: &Tensor, eps: f64) -> Tensor {
        let mean = input.mean(Some(&[0]), true).unwrap();
        let input_shape_binding = input.shape();
        let input_shape = input_shape_binding.dims();
        let mean_expanded = mean.expand(input_shape).unwrap();
        let diff = input.sub(&mean_expanded).unwrap();
        let variance = diff
            .pow_scalar(2.0)
            .unwrap()
            .mean(Some(&[0]), true)
        ...
        let eps_tensor = torsh_tensor::creation::ones_like(&variance)
            ...
            .mul_scalar(eps as f32)
            .unwrap();
        let variance_plus_eps = variance.add(&eps_tensor).unwrap();
        let std = variance_plus_eps.sqrt().unwrap();
        let std_expanded = std.expand(input_shape).unwrap();
        diff.div(&std_expanded).unwrap()
    }
}
...
pub fn dropout(input: &Tensor, p: f64, training: bool) -> Tensor {
    ...
    if p == 1.0 {
        return torsh_tensor::creation::zeros_like(input).unwrap();
    }

    // Create dropout mask using simple thresholding
    let keep_prob = 1.0 - p;
    let random_tensor = torsh_tensor::creation::rand_like(input).unwrap();
    ...
    let diff_tensor = random_tensor.sub(&keep_prob_tensor).unwrap();
    let mask_raw = diff_tensor.relu().unwrap();
    ...
    let keep_data: Vec<f32> = inverted_data
        ...
        .collect();
    let keep_mask =
        torsh_tensor::creation::from_vec(keep_data, input.shape().dims(), input.device()).unwrap();

    // Apply dropout with proper scaling
    let masked = input.mul(&keep_mask).unwrap();
    masked.div_scalar(keep_prob as f32).unwrap()
}
...
pub mod attention {
    ...
    pub fn scaled_dot_product_attention(
        ...
        let d_k = binding.dims().last().unwrap();
        let scale = 1.0 / (*d_k as f64).sqrt();
        let key_transposed = key.transpose(-2, -1).unwrap();
        let scores = query
        ...
        let masked_scores = if let Some(mask) = mask {
            let large_neg = torsh_tensor::creation::ones_like(&scores)
                ...
                .mul_scalar(-1e9_f32)
                .unwrap();
            let mask_effect = mask.mul(&large_neg).unwrap();
            scores.add(&mask_effect).unwrap()
        } else {
            scores
        };

        // Apply softmax and compute attention output
        let attention_weights = masked_scores.softmax(-1).unwrap();
        attention_weights.matmul(value).unwrap()
    }
...
mod tests {
    ...
    fn test_dropout() {
        ...
        let output_vec = output_training.to_vec().unwrap();
        ...
        let output_eval_vec = output_eval.to_vec().unwrap();

...
use crate::registry::{ModelSource, ModelInfo, ModelHandle};
use sha2::Digest;
...
impl ModelDownloader {
    ...
    pub async fn download_model(
        ...
        match &handle.info.source {
            ...
            ModelSource::Url(url) => {
                self.download_from_url(url, &handle.local_path, callback).await
            }
            ModelSource::HuggingFace { repo, filename } => {
                let url = format!("https://huggingface.co/{}/resolve/main/{}", repo, filename);
                self.download_from_url(&url, &handle.local_path, callback).await
            }
            ModelSource::Registry { registry: _, path: _ } => {
    ...
    async fn download_from_url(
        ...
        while let Some(chunk) = stream.next().await {
            ...
            if let Some(ref callback) = callback {
                let now = Instant::now();
                if now.duration_since(last_update).as_millis() >= 100 { // Update every 100ms
...
pub async fn download_model_by_name(
    ...
    downloader.ensure_model_available(&handle, false, callback).await
}

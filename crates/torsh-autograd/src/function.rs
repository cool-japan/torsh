...
impl FunctionContext {
        ...
        self.saved_tensors
            ...
            .map(|t| {
                t.downcast_ref::<Tensor<T>>()
                    .ok_or_else(|| TorshError::AutogradError("Type mismatch in saved tensors".to_string()))
                    .cloned()
            })
            .collect()
    }
    ...
    pub fn get_saved_value<V: Any + 'static>(&self, index: usize) -> Result<&V> {
        self.saved_values
            ...
            .ok_or_else(|| TorshError::AutogradError("Saved value not found or type mismatch".to_string()))
    }
}
...
pub mod functions {
    ...
    impl Function for ReLU {
        ...
        {
            if inputs.len() != 1 {
                return Err(TorshError::AutogradError("ReLU expects exactly one input".to_string()));
            }
    ...
    impl Function for Dropout {
        fn forward<T>(&self, _ctx: &mut FunctionContext, inputs: &[&Tensor<T>]) -> Result<Vec<Tensor<T>>>
        where
            T: torsh_core::dtype::TensorElement,
        {
        ...
        fn backward<T>(&self, _ctx: &mut FunctionContext, grad_outputs: &[&Tensor<T>]) -> Result<Vec<Option<Tensor<T>>>>
        where
            T: torsh_core::dtype::TensorElement,
        {
            if !self.training || self.p == 0.0 {

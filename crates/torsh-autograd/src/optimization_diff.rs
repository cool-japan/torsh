...
impl QuadraticProgrammingLayer {
    ...
    fn adjoint_method_gradient(
        ...
        // Trim adjoint to n_vars.
        let adjoint_x = adjoint.narrow(0, 0, self.n_vars.try_into().expect("n_vars fits i64"))?;

        // Parameter gradients: dL/dθ = adjoint^T · (∂F/∂θ)
        // We mirror the structure of implicit_function_gradient.
        let rhs_q = self.kkt_rhs_q(x, lambda)?;
        let grad_q = adjoint_x.mul(&rhs_q)?;

        let rhs_c = self.kkt_rhs_c()?;
        let grad_c = adjoint_x.mul(&rhs_c)?;

        let rhs_a = self.kkt_rhs_a(lambda)?;
        let grad_a = adjoint_x.mul(&rhs_a)?;

        let rhs_b = self.kkt_rhs_b(lambda)?;
        let grad_b = adjoint_x.mul(&rhs_b)?;

        let rhs_g = self.kkt_rhs_g(mu)?;
        let grad_g = adjoint_x.mul(&rhs_g)?;

        let rhs_h = self.kkt_rhs_h(mu)?;
        let grad_h = adjoint_x.mul(&rhs_h)?;

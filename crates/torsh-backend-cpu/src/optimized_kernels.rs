...
pub fn optimized_matvec(
    ...
    let (expected_vec_len, expected_result_len) = if transpose {
        (m, n)
    } else {
        (n, m) 
    };
...
pub mod advanced {
    ...
    pub fn optimized_conv2d(
        ...
        let col_buffer_size = in_channels * kernel_height * kernel_width * output_height * output_width;
    ...
    fn im2col_cpu(
        ...
        col_buffer.par_chunks_mut(output_h * output_w).enumerate().for_each(|(c, chunk)| {
            ...
            for (h_col, row) in chunk.chunks_mut(output_w).enumerate() {
                for (w_col, out_val) in row.iter_mut().enumerate() {
                    ...
                    if h_im >= pad_h && w_im >= pad_w && 
                       h_im < height + pad_h && w_im < width + pad_w {
    ...
    pub fn optimized_batch_norm(
        ...
        if input.len() != batch_size * channels * spatial_size {
            return Err(TorshError::InvalidArgument("Input size mismatch".to_string()));
        }
        
        output.par_chunks_mut(channels * spatial_size).enumerate().for_each(|(batch, batch_output)| {
            let batch_input = &input[batch * channels * spatial_size..(batch + 1) * channels * spatial_size];
    ...
    pub fn optimized_softmax(
        ...
        output.par_chunks_mut(num_classes).enumerate().for_each(|(batch, batch_output)| {

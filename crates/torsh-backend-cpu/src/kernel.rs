...
use torsh_backends::{
    BackendError, BackendResult, Buffer, Kernel, KernelDescriptor, KernelLaunchConfig,
    Device,
};
...
impl CpuKernel {
    ...
    fn get_cpu_buffer_f32(buffer: &Buffer) -> Result<&[f32]> {
        match &buffer.handle {
            torsh_backends::buffer::BufferHandle::Cpu { ptr, size } => {
                unsafe { Ok(std::slice::from_raw_parts(*ptr as *const f32, size / 4)) }
            },
            _ => Err(TorshError::InvalidArgument("Buffer is not CPU buffer".to_string())),
        }
    }
    ...
    fn get_cpu_buffer_f32_mut(buffer: &Buffer) -> Result<&mut [f32]> {
        match &buffer.handle {
            torsh_backends::buffer::BufferHandle::Cpu { ptr, size } => {
                unsafe { Ok(std::slice::from_raw_parts_mut(*ptr as *mut f32, size / 4)) }
            },
            _ => Err(TorshError::InvalidArgument("Buffer is not CPU buffer".to_string())),
        }
    }
    ...
    pub fn get_kernel_fn(descriptor: &KernelDescriptor) -> Result<CpuKernelFn> {
        let kernel_fn: CpuKernelFn = match descriptor.name.as_str() {
            ...
            _ => return Err(TorshError::InvalidArgument(
                format!("Unsupported kernel: {}", descriptor.name)
            )),
        };
    ...
    fn kernel_add(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 3 {
            return Err(TorshError::InvalidArgument("Add kernel requires 3 buffers".to_string()));
        }
        ...
        if a.len() != b.len() || a.len() != result.len() {
            return Err(TorshError::InvalidArgument("Buffer size mismatch".to_string()));
        }
    ...
    fn kernel_mul(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 3 {
            return Err(TorshError::InvalidArgument("Mul kernel requires 3 buffers".to_string()));
        }
    ...
    fn kernel_sub(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 3 {
            return Err(TorshError::InvalidArgument("Sub kernel requires 3 buffers".to_string()));
        }
    ...
    fn kernel_div(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 3 {
            return Err(TorshError::InvalidArgument("Div kernel requires 3 buffers".to_string()));
        }
    ...
    fn kernel_relu(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 2 {
            return Err(TorshError::InvalidArgument("ReLU kernel requires 2 buffers".to_string()));
        }
    ...
    fn kernel_sigmoid(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 2 {
            return Err(TorshError::InvalidArgument("Sigmoid kernel requires 2 buffers".to_string()));
        }
        ...
        optimized_kernels::parallel_ops::parallel_unary(input, output, |x| 1.0 / (1.0 + (-x).exp()));
        
        Ok(())
    }
    
    /// Tanh activation kernel
    fn kernel_tanh(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 2 {
            return Err(TorshError::InvalidArgument("Tanh kernel requires 2 buffers".to_string()));
        }
    ...
    fn kernel_matmul(buffers: &[&Buffer], uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 3 {
            return Err(TorshError::InvalidArgument("Matmul kernel requires 3 buffers".to_string()));
        }
        ...
        if uniform_data.len() < 14 {
            return Err(TorshError::InvalidArgument(
                "Insufficient uniform data for matmul (need m, n, k, transpose_a, transpose_b)".to_string()
            ));
        }
        
        let m = u32::from_le_bytes([uniform_data[0], uniform_data[1], uniform_data[2], uniform_data[3]]) as usize;
        let n = u32::from_le_bytes([uniform_data[4], uniform_data[5], uniform_data[6], uniform_data[7]]) as usize;
        let k = u32::from_le_bytes([uniform_data[8], uniform_data[9], uniform_data[10], uniform_data[11]]) as usize;
    ...
    fn kernel_dot(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 3 {
            return Err(TorshError::InvalidArgument("Dot kernel requires 3 buffers".to_string()));
        }
        ...
        if result.len() != 1 {
            return Err(TorshError::InvalidArgument("Output buffer should have size 1 for dot product".to_string()));
        }
    ...
    fn kernel_sum(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 2 {
            return Err(TorshError::InvalidArgument("Sum kernel requires 2 buffers".to_string()));
        }
        ...
        if output.len() != 1 {
            return Err(TorshError::InvalidArgument("Output buffer should have size 1 for sum reduction".to_string()));
        }
    ...
    fn kernel_mean(buffers: &[&Buffer], _uniform_data: &[u8], _launch_config: &KernelLaunchConfig) -> Result<()> {
        if buffers.len() != 2 {
            return Err(TorshError::InvalidArgument("Mean kernel requires 2 buffers".to_string()));
        }
        ...
        if output.len() != 1 {
            return Err(TorshError::InvalidArgument("Output buffer should have size 1 for mean reduction".to_string()));
        }
...
mod tests {
    use super::*;
    use torsh_backends::buffer::{BufferDescriptor, BufferUsage, BufferHandle};
    use torsh_backends::kernel::{KernelSource, KernelLanguage};

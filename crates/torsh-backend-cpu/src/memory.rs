...
impl MemoryManager for CpuMemoryManager {
    fn allocate(&mut self, descriptor: &BufferDescriptor) -> Result<Buffer> {
        ...
        let mut pool = self.get_or_create_pool(size_class).map_err(|e| TorshError::AllocationError(e.to_string()))?;
        
        let ptr = pool.allocate(size, alignment).map_err(|e| TorshError::AllocationError(e.to_string()))?;
        
        // Update statistics
        let mut stats = self.stats.lock().map_err(|_| TorshError::AllocationError("Failed to acquire stats lock".to_string()))?;
    ...
    fn deallocate(&mut self, buffer: &Buffer) -> Result<()> {
        ...
        let ptr = match &buffer.handle {
            ...
            _ => return Err(TorshError::AllocationError("Invalid buffer handle type for CPU backend".to_string())),
        };
        
        pool.deallocate(ptr, buffer.size).map_err(|e| TorshError::AllocationError(e.to_string()))?;
...
impl MemoryPool for CpuMemoryPool {
    fn allocate(&mut self, size: usize, alignment: usize) -> Result<*mut u8> {
        let mut free_blocks = self.free_blocks.lock().map_err(|_| TorshError::AllocationError("Failed to acquire free_blocks lock".to_string()))?;
        
        // Try to reuse a free block
        if let Some(ptr) = free_blocks.pop() {
            let mut allocated_blocks = self.allocated_blocks.lock().map_err(|_| TorshError::AllocationError("Failed to acquire allocated_blocks lock".to_string()))?;
        ...
        if ptr.is_null() {
            return Err(TorshError::AllocationError(format!("Failed to allocate {} bytes", size)));
        }
    ...
    fn stats(&self) -> PoolStats {
        let allocated_blocks = self.allocated_blocks.lock().unwrap_or_else(|_| panic!("Lock poisoned"));
        let free_blocks = self.free_blocks.lock().unwrap_or_else(|_| panic!("Lock poisoned"));
        ...
        PoolStats {
            ...
            largest_free_block: if free_blocks.is_empty() { 0 } else { self.size_class },
            smallest_free_block: if free_blocks.is_empty() { 0 } else { self.size_class },
            average_free_block: if free_blocks.is_empty() { 0 } else { self.size_class },
        }
    }

...
use torsh_tensor::{Tensor, creation::{zeros_like, ones_like}};
use std::sync::Arc;
        ...
        for group in &self.param_groups {
            ...
            for param_ref in &group.params {
                ...
                if !self.state.contains_key(&param_ptr) {
                    self.state.insert(param_ptr, AdaMaxState {
    ...
    fn load_state_dict(&mut self, state: OptimizerState) -> Result<()> {
        ...
        for (ptr_str, param_state) in state.state {
            ...
            if let (Some(exp_avg), Some(exp_inf)) = (
                ...
                self.state.insert(dummy_ptr, AdaMaxState {

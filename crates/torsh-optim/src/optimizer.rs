...
impl Optimizer for BaseOptimizer {
    ...
    fn add_param_group(&mut self, params: Vec<Arc<RwLock<Tensor>>>, mut options: HashMap<String, f32>) {
        let lr = options.remove("lr").unwrap_or_else(|| {
            self.defaults.get("lr").copied().unwrap_or(1e-3)
        });
    ...
    fn state_dict(&self) -> OptimizerState {
        let param_groups = self.param_groups.iter()

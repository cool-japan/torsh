...
pub struct NAdam {
    ...
    state: HashMap<*const Tensor, NAdamState>,
    step_count: usize,
}
...
impl Optimizer for NAdam {
        ...
        for group in &self.param_groups {
            ...
            for param_ref in &group.params {
                ...
                if !self.state.contains_key(&param_ptr) {
                    self.state.insert(param_ptr, NAdamState {
                ...
                let m_hat = exp_avg_corrected.mul_scalar(mu_t_next / (1.0 - mu_t_next))?
                    .add(&grad_term)?;
    ...
    fn state_dict(&self) -> OptimizerState {
        let param_groups = self.param_groups.iter().map(|g| ParamGroupState {
            lr: g.lr,
    ...
    fn load_state_dict(&mut self, state: OptimizerState) -> Result<()> {
        ...
        for (ptr_str, param_state) in state.state {
            if let (Some(exp_avg), Some(exp_avg_sq)) = (
                param_state.get("exp_avg"),
                param_state.get("exp_avg_sq")
            ) {
                let dummy_ptr = std::ptr::null() as *const Tensor;
                self.state.insert(dummy_ptr, NAdamState {

...
use torsh_tensor::{Tensor, creation::ones};
use torsh_core::{
...
impl<T: TensorElement> Collate<Vec<Tensor<T>>> for DefaultCollate {
    ...
    fn collate(&self, batch: Vec<Vec<Tensor<T>>>) -> Result<Self::Output> {
        ...
        for i in 0..num_tensors {
            let tensors: Vec<Tensor<T>> = batch.iter()
                .map(|sample| sample[i].clone())
                .collect();
...
mod tests {
    ...
    fn test_custom_collate_fn() {
        let collate = CollateFn::new(|batch: Vec<i32>| {

...
impl BenchRunner {
    ...
    pub fn export_csv(&self, path: &str) -> std::io::Result<()> {
        ...
        writeln!(file, "name,size,dtype,mean_time_ns,std_dev_ns,throughput,memory_usage")?;
    ...
    pub fn generate_report(&self, output_dir: &str) -> std::io::Result<()> {
        ...
        writeln!(file, "<html><head><title>ToRSh Benchmark Report</title></head><body>")?;
...
pub mod prelude {
    ...
    pub use criterion::{Criterion, BenchmarkId, Throughput, black_box};
}
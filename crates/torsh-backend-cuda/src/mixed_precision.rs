...
impl GradientScaler {
    ...
    fn unscale_and_check_tensor(
        ...
        // Check for infinities (would need custom kernel for this)
        // For now, we'll skip the infinity check
...
impl AmpContext {
    ...
    pub fn get_precision(&self, operation: &str) -> DType {
        ...
        match operation {
            // Use FP16 for these operations
            "conv2d" | "linear" | "matmul" | "attention" => DType::F16,
            
            // Use FP32 for these operations (precision sensitive)
            "batch_norm" | "layer_norm" | "softmax" | "cross_entropy" | "mse_loss" => DType::F32,
            
            // Use FP16 for activations
            "relu" | "gelu" | "sigmoid" | "tanh" => DType::F16,
    ...
    pub fn autocast(
        ...
        crate::kernels::tensor_ops::launch_f32_to_f16(
            input.as_raw_mut(),
            output.as_raw_mut(),
            size,
            stream.raw() as cuda_sys::CUstream,
    ...
    pub fn uncast(
        ...
        crate::kernels::tensor_ops::launch_f16_to_f32(
            input.as_raw_mut(),
            output.as_raw_mut(),
            size,
            stream.raw() as cuda_sys::CUstream,
...
impl MixedPrecisionTrainer {
        ...
        F: FnMut(&CudaStream) -> CudaResult<()>,
    {
        ...
        if self.loss_scaling {
            self.amp_context.scaler().scale_gradients(gradients, stream)?;
        }
    ...
    pub fn optimizer_step(
        ...
        if self.loss_scaling {
            self.amp_context.scaler().step(optimizer, gradients, parameters, stream)
        } else {
            optimizer.step(parameters, gradients, stream)?;
            Ok(true)
        }
